...
public class Servicio
{
  ...
  public Response consulta(@FormParam("email") String email) throws Exception
  {
    ...
    try
    {
      PreparedStatement stmt_1 = conexion.prepareStatement("SELECT a.id_usuario,a.email,a.nombre,a.apellido_paterno,a.apellido_materno,a.fecha_nacimiento,a.telefono,a.genero,b.foto FROM usuarios a LEFT OUTER JOIN fotos_usuarios b ON a.id_usuario=b.id_usuario WHERE a.email=?");
      try
      {
  ...
  public Response modifica(@FormParam("usuario") Usuario usuario) throws Exception
  {
    ...
    if (usuario.nombre == null || usuario.nombre.equals(""))
      return Response.status(400).entity(j.toJson(new Error("Es necesario ingresar el nombre"))).build();

    if (usuario.apellido_paterno == null || usuario.apellido_paterno.equals(""))
      return Response.status(400).entity(j.toJson(new Error("Es necesario ingresar el apellido paterno"))).build();

    if (usuario.fecha_nacimiento == null || usuario.fecha_nacimiento.equals(""))
      return Response.status(400).entity(j.toJson(new Error("Es necesario ingresar la fecha de nacimiento"))).build();
    ...
    try
    {
      ...
      if (usuario.foto != null)
      {
        PreparedStatement stmt_2 = conexion.prepareStatement("DELETE FROM fotos_usuarios WHERE email=?");
        try
        {
  ...
  public Response borra(@FormParam("email") String email) throws Exception
  {
    ...
    try
    {
      ...
      stmt_1 = conexion.prepareStatement("DELETE FROM usuarios WHERE email=?");
      try
      {
        stmt_1.setString(1,email);
	      stmt_1.executeUpdate();
